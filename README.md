...
- At a minimum, the following information should be included: `Title`, `Summary`, and `Contact`.
### When should I commit changes?
- When collaborating, you should commit and push a change as soon as you're done making the edit. This will keep the main branch as up to date as possible and will reduce the likelihood of people working from differing versions of the same file. 
...
- Branching is generally used when multiple people want to actively collaborate within the same repo.
  ...
  - You can branch when you want to experiment without affecting the main working version. This works well when you have a functional script and want to add a brainstorm, draft, and develop a new feature. Creating this new feature from the main branch ensures the version that currently works doesn't get altered until the new feature is confirmed to harmoniously integrate into the existing pipeline. If you are doing this, the main branch should generally not be changed until the other feature branch(es) are merged back in. 
### When should I fork?
- Forking is generally not recommended within the scope of RC SFA work. 
- You can fork when you want to use someone else's repo as a starting point to your own new idea or direction. Forking will make a personal copy of another person's repo so that when you make your own edits, it won't affect the original repo. This duplicated copy thus creates two repos; this contrasts with branching, which maintains a single repository.
### When do I create a release?
- You should create a release whenever you want to have an archived "snapshot" recording the state of your repo at a single point in time. A release creates a human readable name associated with a particular commit, allowing you to easily find and compare between versions. A good time to do this is when you want to note the status of your repository at the time of publishing a data package on ESS-DIVE.
